var createPGPwindow = function(){
    path = window.location.href;
    peer = path.split('=');
    peer = peer[peer.length - 1];
    console.log("Trying create PGP chat for peer " + peer);
    return pgpFastChat.selectPeer(peer, event);
};
pgpFastChat = {
    init: function (options) {
        ...
        curFastChat.getSettingsTO = setTimeout(pgpFastChat.getSettings, 300);
    },
    ...
    gotSettings: function(data) {
        ...
        if (curNotifier.is_server) {
            if (!data.im_queue) {
                ...
                curFastChat.lp_error_to = setTimeout(pgpFastChat.updateQueueKeys.bind(pgpFastChat), (curNotifier.error_timeout || 1) * 1000);
    ...
    initLp: function () {
        ...
        pgpFastChat.checkLp();
        curFastChat.checkLpInt = setInterval(pgpFastChat.checkLp, 20000);
    },
    stopLp: function () {
    ...
    clChecked: function (queue, response) {
        ...
        if (response.ts) {
            ...
            pgpFastChat.lcSend('queueSet', {cl_queue: curFastChat.cl_queue});
        }
    ...
    imChecked: function (queue, response) {
        ...
        if (response.ts && curFastChat.im_queue) {
            ...
            pgpFastChat.lcSend('queueSet', {im_queue: curFastChat.im_queue});
        }
        if (!isArray(response.events) || !response.events.length) {
            return;
        }
    ...
    initUI: function () {
        ...
        curFastChat.updateFriendsInt = setInterval(pgpFastChat.clistUpdate, 3 * 60000);
        curFastChat.updateTypingsInt = setInterval(pgpFastChat.updateTypings, 5000);
    ...
    clistShow: function () {
        ...
        if (!curFastChat.clistBox.visible) {
            ...
            pgpFastChat.setActive(curFastChat.clistBox);
            curFastChat.clistBoxScroll && curFastChat.clistBoxScroll.update(false, true);
    ...
    clistPeerOver: function (el, state, e) {
        ...
        if (curFastChat.clSel && state && curFastChat.clSel != id) {
            pgpFastChat.clistPeerOver(ge('fc_contact' + curFastChat.clSel), 0);
        }
    ...
    getCorrespondents: function(q, re, empty) {
        ...
        if (curFastChat.correspondents && curFastChat.correspondents[q] !== undefined) {
            return pgpFastChat.wrapCorrespondents(curFastChat.correspondents[q]) || (empty && '<div class="fc_clist_empty">' + getLang('mail_im_clist_notfound') + '</div>') || '';
        }
        curFastChat.correspondentsTO = setTimeout(pgpFastChat.loadCorrespondents.pbind(q, re), 100);
            ...
            case KEY.ENTER:
                if (e.type != 'keyup' && (el = curFastChat.clSel && ge('fc_contact' + curFastChat.clSel))) {
                    if (e.ctrlKey || e.metaKey && browser.mac) {
                        ...
                        pgpFastChat.selectPeer(curFastChat.clSel);
                    }
                    // fall through
                } else {
                    break;
    ...
    prepareTabIcon: function(peer, opts, noAnim) {
        ...
        if (!mem) {
            var need = 3;
            curFastChat.needPeers[peer] = [need, false, setTimeout(pgpFastChat.getPeers, irand(150, 200)), opts];
    ...
    scrollWrap: function(event) {
        ...
        if (curFastChat.activeBox == curFastChat.clistBox) {
            ...
            pgpFastChat.setPointer(false, curFastChat.pointerMargin, curFastChat.prevPointer);
        } else {
            curFastChat.pointerMargin = -Chat.scrollNode.scrollTop;
            pgpFastChat.setPointer(true, curFastChat.pointerMargin, curFastChat.prevPointer);
        }
    ...
    closeTabIcon: function(peer, ev, nohide) {
        ...
        var onAmin = function() {
            ...
            if (tabEl) {
                tabEl = false;
                if (curFastChat.activeBox) {
                    pgpFastChat.movePointer(curFastChat.activeBox.options.peer, true);
                }
            }
            pgpFastChat.checkChatHeight();
        };
    ...
    movePointer: function(peer, aminate) {
        ...
        if (aminate) {
            if (curFastChat.prevPointer) {
                ...
                var tdiff = pgpFastChat.getPointerShift(true, pm + shift, curFastChat.prevPointer);
    ...
    addPeer: function (peer, events, force, opts) {
        ...
        if (need) {
            if (force) {
                ...
                curFastChat.needPeers[peer] = [need, events, setTimeout(pgpFastChat.getPeers, irand(150, 200)), opts];
                pgpFastChat.lcSend('needPeer', {id: peer, mask: need});
            }
        }
    },
    ...
    addBox: function (peer, data, options) {
        ...
        var wrap = se(rs(pgpFastChat.tplBox, {id: peer, name: data.name, myphoto: Notifier.fixPhoto(curFastChat.me.photo, true), classname: data.online ? (data.online > 0 && data.online < 6 ? ' fc_tab_mobile' : ' fc_tab_online') : '', cont: editCont}));
    ...
    prepareMsgData: function (arr) {
        ...
        if (attFlags) { // Media
            ...
            if (attFlags & 6) {
                attText += rs(curFastChat.tpl.msg_fwd, {msg_id: arr[1], peer_nice: pgpFastChat.nicePeer(peer), label: getLang(attFlags & 2 ? 'mail_im_fwd_msg' : 'mail_im_fwd_msgs')});
            }
            if (attFlags & 8) {
                data.sticker = true;
            }
    ...
    needMsgMedia: function (peer, msgId) {
        ...
        curFastChat.needMedia[msgId] = [peer, setTimeout(pgpFastChat.loadMsgMedia.pbind(peer, msgId), curNotifier.is_server ? 0 : irand(150, 250))];
    },
